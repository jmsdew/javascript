...
const foodList = ['물회', '삼계탕', '냉면', '수박', '물회'];  // indexof - 가장 앞에서부터 찾는것, 두번째 인자는 여기서는 1은 건너뛴다는 뜻
...
console.log(`foodList.lastIndexOf('물회') : ${foodList.lastIndexOf('물회')}`);          // 4   마지막부터 찾는것
...
console.log(`foodList.includes('물회') : ${foodList.includes('물회')}`);                // true  배열에 들어있는지
...
console.log(`chineseFood.pop() : ${chineseFood.pop()}`);        // 양장피  마지막 요소 반환하고 제거
...
chickenList.unshift('간장치킨');  // 배열의 앞에 추가하는것. 성능상 이슈 있음
...
// slice(시작인덱스, 종료인덱스)
// slice - 종료인덱스 : 요소의 값은 반환하지 않는다. 1<= ㅜ < 3
